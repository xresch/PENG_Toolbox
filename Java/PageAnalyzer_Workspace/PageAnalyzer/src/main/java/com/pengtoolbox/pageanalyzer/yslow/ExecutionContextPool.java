...
public class ExecutionContextPool {
	...
	public static ExecutionContext lockContext() {
		...
		new CFWLogger(logger).method("releaseExecutor")
		...
		while(context == null) {
			...
			if(context == null) {
				try {
					Thread.sleep(200);
					new CFWLogger(logger).method("lockExecutor").fine("Thread waiting for free executor.");
				} catch (InterruptedException e) {
					new CFWLogger(logger).method("lockExecutor").warn("Thread interrupted while taking a nap.", e);
					e.printStackTrace();
				}
			}
		}
