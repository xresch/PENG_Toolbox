...
public class BytecodeTransformer implements ClassFileTransformer {
	...
	public byte[] transform(ClassLoader loader, String className,
		...
		if (className.equals("com/pengtoolbox/pageanalyzer/response/AbstractTemplateHTML")) {
			byteCode = adjustHTMLTemplate(loader, className, classfileBuffer, byteCode); 
	 ...
	 * 
	 *******************************************************************************/
	private byte[] adjustHTMLTemplate(ClassLoader loader, String className, byte[] classfileBuffer, byte[] byteCode) {
		...
		try {
			...
			printHtml.setBody(
					"{ java.io.PrintWriter myOut = $1; "+
					  "try{"
						+ "java.io.File file = new java.io.File(\""+InjectionAgent.CUSTOM_DIR_PATH+"/content.html\");"
